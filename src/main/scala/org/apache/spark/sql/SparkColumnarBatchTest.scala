...
class SparkColumnarBatchTest (fioOptions:FIOOptions, spark:SparkSession) extends BaseTest {
  ...
  override def execute(): String = {
    ...
    "ColumnarBatch : Parquet<int,payload> read " + filesEnumerated.size + " HDFS files in " + fioOptions.getInputLocations + " directory (total bytes " + totalBytesExpected + " ), total rows " + totalRows.value + " rowBatch " + rowBatches.value
  }
