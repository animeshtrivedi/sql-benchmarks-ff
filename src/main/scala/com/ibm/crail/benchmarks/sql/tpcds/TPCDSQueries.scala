...
case class TPCDSQuery(id:String, query:String)

object TPCDSQueries {
  val TOTAL_QUERIES = 104
  def lookup(id:String):TPCDSQuery = {
    var i = 0
    var found = false
    while (!found && i < TOTAL_QUERIES) {
      if(queries(i).id.compareToIgnoreCase(id) == 0) {
        found = true
      }
      i+=1
    }
    if(i == TOTAL_QUERIES)
      throw new Exception("Illegal query name: " + id)

    queries(i)
  }

  // fixed number
  val queries = Array(TPCDSQuery("q10.sql",
    ("" +
      "" +
      " select" +
  ...
  require(this.queries.length == TOTAL_QUERIES)
}
